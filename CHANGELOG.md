...
- **UR A.16 隱身模式（API＋DB＋UI）[✓，用户已验收，merged]**
  ...
  - **UR C.2 v2 模式選擇顯性化 [✓，用户已验收，merged]**：Popover退役，常駐圖標＋文字pill＋inline三檔直切（選後自動收／再點收；匿名走登入）；零新key；三閘241綠／lint 0 error／build 39頁：全量新文件（`/v2`＋`components/v2`＋v2 css作用域＋v2三語11鍵parity PASS）；OSM免key底圖（CARTO要key不碰）；選酒L1／L2／雙鈕／乾杯邀約守衛／足跡／搖一搖／模式Popover全接共用層；v1零文件（隔離門過）；單測v2Pins 4測；三閘241綠／lint 0 error／build 39頁
...
- **URC 1.1 — 縮放按鈕退役，改用雙指捏合（A3 桌面 hover 滾輪）（[✓]，merged）**
  - `MapToolbar` 砍 +/- 縮放鈕，6 鈕變 4 鈕（睇全港／回位／足跡＋搖一搖衛星）；`handleZoom` 退役（無調用點）
  - `DrinkMap`：縮放改三路——(1) 手機／平板雙指捏合已可用（CSS `touch-action: pan-y pinch-zoom` + Leaflet 默認，UR1.3 留下）；(2) 桌面 hover 地圖才啟用滾輪縮放（`map.scrollWheelZoom.enable()`／`.disable()` 後切，UR1.3 scroll-trap 行為給頁面其他部分保留）；(3) 鍵盤 +/- 鍵（Leaflet `keyboard: true` 內建）
