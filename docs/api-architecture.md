...
1. 登入方式：**Google OAuth 一鍵登入（免密碼）**（2026-09-10 用戶推翻匿名先行：
   Google 帳號＝email，OTP／密碼全不要；匿名→登入認領合併大坑一步跨過。
   原「匿名設備號先行」作廢；公開讀保持匿名可看，寫操作一律要登入）。
2. 語音保留期：**90 天自動清**，轉錄文字永久留（符合 PDPO 最小化）。
3. 匿名期數據：**不認領、不合併**（Q1 推翻連帶：本地 localStorage 數據不進庫；
   `devices` 表留待推送登記 A.4-15 用）。
